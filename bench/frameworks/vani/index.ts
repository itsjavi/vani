  ...
  return () =>
    div(
      ...
      button(
        {
          type: 'button',
          className: { 'bench-view-btn': true, 'is-active': currentView === 'datatable' },
      ...
      button(
        {
          type: 'button',
          className: { 'bench-view-btn': true, 'is-active': currentView === 'pokeboxes' },
      ...
      div(
        ...
        div(
          ...
          div(
            ...
            h1({ className: 'bench-title mb-0' }, title),
          ),
          div(
            { className: 'col-lg-6' },
            div(
...
const BoxCard = component<{ boxIndex: number; entry: BoxEntry }>((props) => {
  return () =>
    div(
      { className: 'pokebox-card', 'data-box-index': String(props.boxIndex) },
      div(
        { className: 'pokebox-header' },
        span({ className: 'pokebox-title' }, `Box ${props.entry.id}`),
        button({
          type: 'button',
...
const PokeboxGrid = component((_, handle: Handle) => {
  ...
  return () => div({ className: 'pokebox-grid', id: 'pokebox-grid', ref: pokeboxGridRef })
})
...
renderToDOM(view === 'pokeboxes' ? PokeboxApp() : DataTableApp(), rootNode)
