...
const ViewToggle = ({ currentView }: { currentView: View }) => (
  <div className={cn('bench-view-toggle')} id="view-toggle">
    <button
      type="button"
      className={cn('bench-view-button', {
...
const Header = ({
  ...
  <div className={cn('bench-hero', 'bench-section')}>
    <div className={cn('bench-grid-2')}>
      <div className={cn('relative')}>
        <ViewToggle currentView={currentView} />
        <h1 className={cn('bench-title')}>{title}</h1>
      </div>
      <div>
        <div className={cn('bench-actions')} id="app-actions">
          {actions.map((action) => (
            <button
              id={action.id}
              className={cn('bench-button', 'bench-button-primary', 'w-full')}
              type="button"
...
function DataTableApp() {
  ...
  return (
    <div className={cn('bench-container')}>
      <Header title="React" currentView="datatable" actions={actions} />
      <table className={cn('bench-data-table')}>
        <tbody id="tbody">
          {rows.map((row) => {
            let rowId = row.id
            return (
              <tr
                key={rowId}
                className={cn('bench-data-row', { 'bench-data-active': selected === rowId })}
              >
                <td className={cn('bench-data-cell', 'bench-data-id')}>{rowId}</td>
                <td className={cn('bench-data-cell')}>
                  <a
                    className={cn('bench-data-link')}
                    href="/"
                    onClick={(event) => {
                ...
                <td className={cn('bench-data-cell')}>
                  <button
                    className={cn('bench-data-remove')}
                    type="button"
                    aria-label="Remove"
                    onClick={(event) => {
                      event.preventDefault()
      ...
      <span className={cn('bench-preload-icon', 'bench-data-remove')} aria-hidden="true" />
    </div>
  )
}
...
function PokeboxApp() {
  ...
  return (
    <div className={cn('bench-container')}>
      ...
      <div className={cn('bench-pokebox-grid')} id="pokebox-grid">
        {boxes.map((box, boxIndex) => (
          <div className={cn('bench-pokebox-card')} key={box.id} data-box-index={boxIndex}>
            <div className={cn('bench-pokebox-header')}>
              <span className={cn('bench-pokebox-title')}>Box {box.id}</span>
              <button
                type="button"
                className={cn('bench-pokebox-remove')}
                aria-label={`Remove Box ${box.id}`}
                data-box-index={boxIndex}
                onClick={() => removeBox(boxIndex)}
              />
            </div>
            <div className={cn('bench-pokebox-cells')}>
              {box.cells.map((cell, cellIndex) => {
                ...
                return (
                  <button
                    type="button"
                    className={cn('bench-pokebox-cell')}
