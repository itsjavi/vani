...
// THIS IS VANILLA JS, using TS for convenient imports.
// KEEP THE TYPES TO A MINIMUM !!

const viewToggle = document.querySelector<HTMLButtonElement>('#view-toggle')
const appActions = document.querySelector<HTMLDivElement>('#app-actions')
let table = document.querySelector<HTMLTableElement>('.bench-data-table')
let pokeboxGrid = document.querySelector<HTMLDivElement>('#pokebox-grid')
const preloadIcon = document.querySelector<HTMLSpanElement>('.bench-preload-icon')
...
const renderViewToggle = (currentView) => {
  ...
  const buildButton = (view, icon, label) => {
    ...
    button.type = 'button'
    button.className = `bench-view-button${currentView === view ? ' bench-view-button-active' : ''}`
...
const renderActions = (actions) => {
  ...
  for (const action of actions) {
    const button = document.createElement('button')
    button.type = 'button'
    button.className = 'bench-button bench-button-primary w-full'
    button.id = action.id
    button.textContent = action.label
    appActions.appendChild(button)
  }
}
...
const initDataTableView = () => {
  ...
  let rowById = new Map<number, Element>()
  let labelById = new Map<number, Element>()
  let selectedRowEl = null

  const createRow = (row: Row) => {
    const cloned = rowTemplate.cloneNode(true) as HTMLElement
    ...
    if (selectedId === row.id) cloned.classList.add('bench-data-active')
    return cloned
  }
  ...
  const updateSelectedRow = (nextSelectedId) => {
    if (selectedRowEl) {
      selectedRowEl.classList.remove('bench-data-active')
    }
    selectedId = nextSelectedId
    selectedRowEl = selectedId === null ? null : (rowById.get(selectedId) ?? null)
    if (selectedRowEl) selectedRowEl.classList.add('bench-data-active')
  }
  ...
  tbody.addEventListener('click', (event) => {
    ...
    if (target.closest('.bench-data-link')) {
    ...
    if (target.closest('.bench-data-remove')) {
      event.preventDefault()
      const id = Number(rowElement.dataset.id)
      rows = remove(rows, id)
      rowElement.remove()
...
const initPokeboxView = () => {
  ...
  if (!pokeboxGrid) {
    ...
    pokeboxGrid.className = 'bench-pokebox-grid'
  ...
  pokeboxGrid.addEventListener('click', (event) => {
    ...
    const removeButton = target.closest('.bench-pokebox-remove')
    ...
    const cellButton = target.closest('.bench-pokebox-cell')
