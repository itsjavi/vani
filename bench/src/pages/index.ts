...
import { getFrameworks, indexNotes, pkgJson } from '../metadata'
...
const buildScoreBadge = (frameworkId?: string) => {
  ...
  return `<span class="bench-score-badge ${scoreBadgeClass(score, best)}">${score.toFixed(
    1,
  )} ms</span>`
}
...
const notesToParagraph = (notes: string) => {
  return `<div class="bench-card text-pretty">
  <div class="bench-card-header">
    <h3 class="bench-card-title">Implementation Notes</h3>
  </div>
  <div class="bench-card-body bench-stack">
    ${notes
      .split('\n\n')
      .map(
        (paragraph) =>
          `<p class="text-sm text-muted">${paragraph
            .split('\n')
...
<div class="bench-container bench-section">
  <div class="bench-page-header">
    <div class="bench-stack">
      <h1 class="bench-page-title">
        Frontend Framework Benchmarks
        <span class="bench-page-version">v${pkgJson.benchmarks.version}</span>
      </h1>
  ...
  <div class="bench-grid-2">
      ...
      <div class="bench-card">
        ...
        <div class="bench-list">
          ${sortedFrameworks
            .map(
              (framework) =>
                `<a class="bench-list-item" href="./${framework?.path}">
                  <span class="bench-item-title">${framework?.name} <span class="bench-item-meta">(v${framework?.version})</span></span>
    ...
    <div class="bench-stack">
      <div class="bench-card">
        <div class="bench-card-header">
          <h3 class="bench-card-title">Last Benchmark Results</h3>
        </div>
        <div class="bench-card-body bench-stack">
          <p class="text-muted">Open the latest snapshot table and breakdown.</p>
          <a class="bench-button bench-button-primary w-full" href="./results">
            View results
          </a>
        </div>
      </div>
      ...
      <div class="bench-card">
        <div class="bench-card-header">
          <h3 class="bench-card-title">Manual UI Tests</h3>
        </div>
        <div class="bench-list">
          ${manualTests
            .map(
              (test) =>
                `<a class="bench-list-item" href="./${test?.path}">
                  <span class="bench-item-title">${test?.name}</span>
                </a>`,
            )
            .join('')}
        </div>
  ...
  <footer class="bench-footer bench-footer-row">
    <span>Last bench run: <span id="last-bench-run">Loading...</span></span>
    <a class="bench-data-link" href="https://vanijs.dev" target="_blank" rel="noreferrer">
      vanijs.dev
    </a>
  </footer>
</div>
